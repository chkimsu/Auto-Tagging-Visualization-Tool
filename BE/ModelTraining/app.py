...
print("Check command_at_inference_CHANGED_sql:",command_at_inference_CHANGED_sql)  # Check command_at_inference_CHANGED_sql
...
command_at_inference_CHANGED_sql2 = f.iloc[13,1]
print("Check command_at_inference_CHANGED_sql2:", command_at_inference_CHANGED_sql2)  # Check command_at_inference_CHANGED_sql2
content_type = f.iloc[14,1]
print("Check content_type:", content_type)  # Check content_type
url = f.iloc[15,1]
print("Check url:", url)  # Check url
...
def predict(text):
    ...
    if str(predictions_xlnet[0]) == '10':
        ...
        result = "idk idk" #"exhaust air fan cmd"
    else:
        result = "exhaust air fan cmd"#"idk"
...
def inference():
    ...
    update_sql = "UPDATE t_points_atvt SET point = '" + return_value + \
...
def command_at_inference_ALL():
    ...
    for i in range(len(rows_rows)):
        if rows_rows['id'].iloc[i] == "null" or rows_rows['id'].iloc[i] == "none" :
            return_value['equip'].iloc[i] = "idk"
            return_value['point'].iloc[i] = "idk"
            return_value['tagged'].iloc[i] = "y"
            return_value['changed'].iloc[i] = "n"
            return_value['inferenced'].iloc[i] = "f"
            return_value['model_version'].iloc[i] = str(datetime.now().strftime('%Y.%m.%d')) + ".v1"
            return_value['id'].iloc[i] = rows_rows['id'].iloc[i]
            return_value['name'].iloc[i] = rows_rows['name'].iloc[i]
            return_value['description'].iloc[i] = rows_rows['description'].iloc[i]
            return_value['equip_id'].iloc[i] = rows_rows['equip_id'].iloc[i]
            return_value['present_value'].iloc[i] = rows_rows['present_value'].iloc[i]
            return_value['inference_time'].iloc[i] = str(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
            return_value['create_time'].iloc[i] = rows_rows['create_time'].iloc[i]
            return_value['update_time'].iloc[i] = rows_rows['update_time'].iloc[i]

        elif rows_rows['name'].iloc[i] == "null" or rows_rows['name'].iloc[i] == "none":
            return_value['equip'].iloc[i] = "idk"
            return_value['point'].iloc[i] = "idk"
            return_value['tagged'].iloc[i] = "y"
            return_value['changed'].iloc[i] = "n"
        ...
        elif rows_rows['description'].iloc[i] == "null" or rows_rows['description'].iloc[i] == "none":
            return_value['equip'].iloc[i] = "idk"
            return_value['point'].iloc[i] = "idk"
            return_value['tagged'].iloc[i] = "y"
            return_value['changed'].iloc[i] = "n"
        ...
        elif rows_rows['present_value'].iloc[i] == "null" or rows_rows['present_value'].iloc[i] == "none":
            return_value['equip'].iloc[i] = "idk"
            return_value['point'].iloc[i] = "idk"
            return_value['tagged'].iloc[i] = "y"
            return_value['changed'].iloc[i] = "n"
        ...
        elif predict(rows_rows['id'].iloc[i]) == "idk idk":
            return_value['equip'].iloc[i] = "idk"
            return_value['point'].iloc[i] = "idk"
            return_value['tagged'].iloc[i] = "n"
            return_value['changed'].iloc[i] = "n"
            return_value['inferenced'].iloc[i] = "f"
            return_value['model_version'].iloc[i] = str(datetime.now().strftime('%Y.%m.%d')) + ".v1"
            return_value['id'].iloc[i] = rows_rows['id'].iloc[i]
            ...
            # object_id_value1 = rows_rows1['id'].iloc[i]
            result = predict(rows_rows['id'].iloc[i])
            equip, space, point = result.partition(' ')
            return_value['equip'].iloc[i] = equip
            return_value['point'].iloc[i] = point
            return_value['tagged'].iloc[i] = "y"
            return_value['changed'].iloc[i] = "n"
            return_value['inferenced'].iloc[i] = "s"
            return_value['model_version'].iloc[i] = str(datetime.now().strftime('%Y.%m.%d')) + ".v1"
            return_value['id'].iloc[i] = rows_rows['id'].iloc[i]
            return_value['name'].iloc[i] = rows_rows['name'].iloc[i]
            return_value['description'].iloc[i] = rows_rows['description'].iloc[i]
    ...
    for i in range(len(return_value)):
        res = requests.post(url, headers=headers, data=return_value.iloc[i, :].to_json())
        print(return_value.iloc[i, :].to_json())
        print(res)

    # return 'Success'
    #     update_sql1 = "UPDATE t_point_atvt SET point = '" + return_value1 + "', tagged = 'y', changed = 'n', changed = 'f', inference_time = '" + str(
    ...
    return "command_at_inference_ALL is done"
...
def command_at_inference_CHANGED():
    ...
    conn1 = pymysql.connect(host="192.168.21.6", user="root", password="ntels",
                                   db="nise", charset="utf8", cursorclass=pymysql.cursors.DictCursor)
    curs1 = conn1.cursor()
    ...
    rows_rows1.head()
    return_value = pd.DataFrame(index=range(0, len(rows_rows1)),columns=['tagged','changed','inferenced','model_version','id','description','name','point','equip','equip_id','present_value','inference_time','create_time','update_time'])
    for i in range(len(rows_rows1)):
        if rows_rows1['id'].iloc[i] == "null" or rows_rows1['id'].iloc[i] == "none":
            return_value['equip'].iloc[i] = "idk"
            return_value['point'].iloc[i] = "idk"
            return_value['tagged'].iloc[i] = "y"
            return_value['changed'].iloc[i] = "n"
            return_value['inferenced'].iloc[i] = "f"
            return_value['model_version'].iloc[i] = str(datetime.now().strftime('%Y.%m.%d'))+".v1"
            return_value['id'].iloc[i] = rows_rows1['id'].iloc[i]
            return_value['name'].iloc[i] = rows_rows1['name'].iloc[i]
            return_value['description'].iloc[i] = rows_rows1['description'].iloc[i]
            return_value['equip_id'].iloc[i] = rows_rows1['equip_id'].iloc[i]
            return_value['present_value'].iloc[i] = rows_rows1['present_value'].iloc[i]
            return_value['inference_time'].iloc[i] = str(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
            return_value['create_time'].iloc[i] = rows_rows1['create_time'].iloc[i]
            return_value['update_time'].iloc[i] = rows_rows1['update_time'].iloc[i]

        elif rows_rows1['name'].iloc[i] == "null" or rows_rows1['name'].iloc[i] == "none":
            return_value['equip'].iloc[i] = "idk"
            return_value['point'].iloc[i] = "idk"
            return_value['tagged'].iloc[i] = "y"
            return_value['changed'].iloc[i] = "n"
        ...
        elif rows_rows1['description'].iloc[i] == "null" or rows_rows1['description'].iloc[i] == "none":
            return_value['equip'].iloc[i] = "idk"
            return_value['point'].iloc[i] = "idk"
            return_value['tagged'].iloc[i] = "y"
            return_value['changed'].iloc[i] = "n"
        ...
        elif rows_rows1['present_value'].iloc[i] == "null" or rows_rows1['present_value'].iloc[i] == "none":
            return_value['equip'].iloc[i] = "idk"
            return_value['point'].iloc[i] = "idk"
            return_value['tagged'].iloc[i] = "y"
            return_value['changed'].iloc[i] = "n"
        ...
        elif predict(rows_rows1['id'].iloc[i]) == "idk idk":
            return_value['equip'].iloc[i] = "idk"
            return_value['point'].iloc[i] = "idk"
            return_value['tagged'].iloc[i] = "y"
            return_value['changed'].iloc[i] = "n"
        ...
        # object_id_value1 = rows_rows1['id'].iloc[i]
            result = predict(rows_rows1['id'].iloc[i])
            equip, space, point = result.partition(' ')
            return_value['equip'].iloc[i] = equip
            return_value['point'].iloc[i] = point
            return_value['tagged'].iloc[i] ="y"
   ...
   #  conn1.close()
    ...
    return 'command_at_inference_CHANGED is Success'

def command_at_inference_CHANGED2():
    """command to command_at_inference_CHANGED
                   Args:
                     none
                   SQL_STATUE:
                   ...
                   Returns:
                     result : (type) int // Success result
    """
    print("command_at_inference_CHANGED2 is start")  # command_at_inference_CHANGED fucntion start
    conn1 = pymysql.connect(host="192.168.21.6", user="root", password="ntels",
                                   db="nise", charset="utf8", cursorclass=pymysql.cursors.DictCursor)
    curs1 = conn1.cursor()
    curs1.execute(command_at_inference_CHANGED_sql2)
    rows1 = curs1.fetchall()
    print(rows1)
    rows_rows1 = pd.DataFrame(rows1)
    rows_rows1.head()
    ...
    #  conn1.close()
    conn1.close()
    print("command_at_inference_CHANGED2 is done")  # command_at_inference_CHANGED fucntion done
    return 'command_at_inference_CHANGED2 is Success'
...
@ns.route('/command/at-inference', methods = ['POST'])  # Flask Route setting
...
class Test(Resource):
    ...
    def post(self):
        ...
        if(content['command'] == 'ALL'):
          ...
          result = command_at_inference_CHANGED()
          result = command_at_inference_CHANGED2()
          return result

# @ns.route('/commandatinference/<string:text>')  # Flask Route setting
...
#                          result : result message
#         """
#         print("commandatinference_post is start")  # commandatinference_post start
#         if (text == 'ALL'):
...
#
# @ns.route('/prediction/train_external_model/')  # Flask Route setting
# @ns.response(404, '네트워크가 불안정 합니다.')  # Flask Response setting
# @ns.param('train_external_model', 'train_external_model')  ##Flask param setting
# class trainexternalmodel(Resource):
#     """Flask API Class-trainexternalmodel.
#            Properties:
#             post() : Message for post type
#     """
#
...
#                          result : result message
#         """
#         print("trainexternalmodel_post is start")  # trainexternalmodel_post start
#         result = train_external_model()
#         print("trainexternalmodel_post is done")  # trainexternalmodel_post done
#         return result
